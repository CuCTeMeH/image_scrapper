...
def resized_path(path, size, method):
    ...
    return os.path.join(dir + '/resized/', '%s_%s_%s.%s' % (image_name, method, size, EXT))
...
def scale(imagefield, size, method='scale'):
    ...
    path = '/media/D/virtual_env/bin/melon/melon/static/images/' + imagefield.path.split('/')[-1]
    image_path = resized_path(path, size, method)
